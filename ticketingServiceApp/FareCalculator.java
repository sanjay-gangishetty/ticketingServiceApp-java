...
//	calculates fare by seats and passengers and calls bill printer method
	static void fareCalculator(Scanner sc, String source, int price, int seats, String journeyDate) {
		try {
			...
			seats -= passengers;
			if (seats > 0) {
				BillPrinter.billPrinter(sc, source, passengers, price, journeyDate);
			} else {
				System.out.println("Only " + seats + " Seats are Empty");
				System.out.println("Please reduce passengers and retry!!");
				fareCalculator(sc, source, price, seats, journeyDate);
			}
